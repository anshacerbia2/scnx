...
export default function Carousel_01({ images, content, timer }) {
  ...
  const [slotSettin2g, setSlotSettin2g] = useState({
    slotItemStart: true,
  ...
  const slotMaker = () => {
    ...
    for (let i = 0; i < slotRow * slotCol; i++) {
      const left =
        (i - (Math.ceil((i + 1) / slotCol) - 1) * slotCol) * slotSize;
        ...
        >
          <div
            style={{
              ...
              width,
              height,
              transform: slotSetting.slotItemImgTransformStart
                ? `translate3d(-${slotSize}px, -${slotSize}px, 0)`
                : "translate3d(0, 0, 0)",
  ...
  return (
    <div className={styles["c-01-display"]} ref={parentRef}>
      ...
      />
      <div className={styles["c-01-slot-holder"]}>
        {clearSlot ? <></> : slotMaker()}
      </div>
